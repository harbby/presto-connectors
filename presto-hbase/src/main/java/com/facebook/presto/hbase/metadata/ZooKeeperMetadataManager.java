...
public class ZooKeeperMetadataManager
{
    ...
    public ZooKeeperMetadataManager(HbaseConfig config, TypeManager typeManager)
    {
        ...
        objectMapperProvider.setJsonDeserializers(ImmutableMap.of(Type.class, new HbaseModule.TypeDeserializer(typeManager)));
        ...
        if (exists) {
            try {
                ...
                tables.addAll(
                        curator.getChildren().forPath(schemaPath)
                                .stream()
                                .filter(x -> isHbaseView(new SchemaTableName(schema, x)))
                                .collect(Collectors.toList()));
                return tables;
            }
            catch (Exception e) {
    ...
    private boolean isHbaseTable(byte[] data)
            throws IOException
    {
    ...
    private boolean isHbaseView(byte[] data)
            throws IOException
    {
    ...
    private HbaseTable toHbaseTable(byte[] data)
            throws IOException
    {
        return mapper.readValue(new String(data), HbaseTable.class);
    }

    private HbaseView toHbaseView(byte[] data)
            throws IOException
    {
